...
static const BelugaSerialBase::target_pair cmu_beluga = {"CMU", "Beluga"};
static const BelugaSerialBase::target_pair segger_jlink = {"SEGGER", "J-Link"};

static const std::vector<BelugaSerialBase::target_pair> TARGETS = {
    cmu_beluga, segger_jlink};

constexpr auto open_delay = 500ms;

static const std::map<BelugaSerialBase::target_pair, bool> USB_STILL_ALIVE = {
...
BelugaSerialBase::BelugaSerialBase() { _initialize(); }

BelugaSerialBase::BelugaSerialBase(const BelugaSerialAttributes &attr) {
    _initialize(attr);
}

void BelugaSerialBase::_initialize(const BelugaSerialAttributes &attr) {
        ...
        for (const auto &target : TARGETS) {
            ...
            for (const auto &port_ : avail_ports[target]) {
                ...
                try {
                    ...
                    _usb_remains_open = iterator->second;
                    break;
...
void BelugaSerialBase::_publish_response(std::string &response) {
...
void BelugaSerialBase::_find_ports(
    const std::vector<BelugaSerialBase::target_pair> &valid,
    std::map<BelugaSerialBase::target_pair, std::vector<std::string>>
        &avail_ports) {
...
void BelugaSerialBase::_process_frames_helper() {
    while (_tasks_running) {
        ...
        case BelugaFrame::BelugaFrameType::NEIGHBOR_DROP:
            remove_from_neighbor_list_(std::get<uint32_t>(frame.payload));
            break;
        ...
        publish_neighbor_update_();
        publish_range_update_();
    }
}

void BelugaSerialBase::_process_frames() {
    while (_tasks_running) {
        try {
...
void BelugaSerialBase::_process_rx_buffer(std::vector<uint8_t> &buf) {
    while (true) {
...
void BelugaSerialBase::_read_serial_helper() {
...
void BelugaSerialBase::_read_serial() {
    while (_tasks_running) {
        try {
            _read_serial_helper();
...
std::string BelugaSerialBase::_send_command(const std::string &command) {
...
std::string BelugaSerialBase::start_uwb() {
    return _send_command("AT+STARTUWB\r\n");
}

std::string BelugaSerialBase::stop_uwb() {
    return _send_command("AT+STOPUWB\r\n");
}

std::string BelugaSerialBase::start_ble() {
    return _send_command("AT+STARTBLE\r\n");
}

std::string BelugaSerialBase::stop_ble() {
    return _send_command("AT+STOPBLE\r\n");
}

std::string BelugaSerialBase::id(const std::string &id) {
    std::stringstream oss;
    std::string ret;
    oss << "AT+ID " << id << "\r\n";
    ret = _send_command(oss.str());

    if (!id.empty() && ret.ends_with("OK")) {
        _id = _extract_id(oss.str());
    }
    return ret;
}

std::string BelugaSerialBase::bootmode(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+BOOTMODE " << mode << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::rate(const std::string &rate) {
    std::stringstream oss;
    oss << "AT+RATE " << rate << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::channel(const std::string &channel) {
    std::stringstream oss;
    oss << "AT+CHANNEL " << channel << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::reset() { return _send_command("AT+RESET\r\n"); }

std::string BelugaSerialBase::timeout(const std::string &timeout) {
    std::stringstream oss;
    oss << "AT+TIMEOUT " << timeout << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::txpower(const std::string &power) {
    std::stringstream oss;
...
std::string BelugaSerialBase::twrmode(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+TWRMODE " << mode << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::ledmode(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+LEDMODE " << mode << "\r\n";
    return _send_command(oss.str());
}

void BelugaSerialBase::_reboot() {
...
std::string BelugaSerialBase::reboot() {
    std::string ret;
    if (_usb_remains_open) {
        _reboot_done.clear();
...
std::string BelugaSerialBase::pwramp(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+PWRAMP " << mode << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::antenna(const std::string &antenna) {
    std::stringstream oss;
    oss << "AT+ANTENNA " << antenna << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::time() { return _send_command("AT+TIME\r\n"); }

std::string BelugaSerialBase::_format(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+FORMAT " << mode << "\r\n";
    return _send_command(oss.str());
}
...
std::string BelugaSerialBase::datarate(const std::string &rate) {
    std::stringstream oss;
    oss << "AT+DATARATE " << rate << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::preamble(const std::string &preamble) {
    std::stringstream oss;
    oss << "AT+PREAMBLE " << preamble << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::pulserate(const std::string &pr) {
    std::stringstream oss;
    oss << "AT+PULSERATE " << pr << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::phr(const std::string &phr) {
    std::stringstream oss;
    oss << "AT+PHR " << phr << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::pac(const std::string &pac) {
    std::stringstream oss;
    oss << "AT+PAC " << pac << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::sfd(const std::string &sfd) {
    std::stringstream oss;
    oss << "AT+SFD " << sfd << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::panid(const std::string &pan_id) {
    std::stringstream oss;
    oss << "AT+PANID " << pan_id << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::evict(const std::string &scheme) {
    std::stringstream oss;
    oss << "AT+EVICT " << scheme << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::verbose(const std::string &mode) {
    std::stringstream oss;
    oss << "AT+VERBOSE " << mode << "\r\n";
    return _send_command(oss.str());
}

std::string BelugaSerialBase::status() {
    return _send_command("AT+STATUS\r\n");
}

std::string BelugaSerialBase::version() {
...
void BelugaSerialBase::register_resync_cb(std::function<void()> cb) {
    _time_resync = std::move(cb);
}

void BelugaSerialBase::swap_port(const std::string &port) {
    fs::path path = port;
...
uint16_t BelugaSerialBase::_extract_id(const std::string &s) {
...
std::vector<std::string> BelugaSerialBase::_find_port_candidates(
...
bool BelugaSerialBase::_open_port(std::string &port) {
    bool ret = true;
...
std::string BelugaSerialBase::_get_id_from_device() {
...
void BelugaSerialBase::_reconnect_helper() {
...
void BelugaSerialBase::_reconnect() {
    std::lock_guard<std::recursive_mutex> lock(_serial_lock);
    std::packaged_task<void()> task([this] { _reconnect_helper(); });

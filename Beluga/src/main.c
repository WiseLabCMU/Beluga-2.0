...
hrs_measurement_notify_cb(struct bt_hrs_client *hrs_c,
                          ...
                          int err) {
    if (err) {
        printk("Error during receiving Heart Rate Measurement notification, "
               "err: %d\n",
               err);
        return;
    }
    ...
    if (err) {
        printk("Notification queue is full. Discarting HRS notification (err "
               "%d)\n",
               err);
    }
}
...
static void discovery_completed_cb(struct bt_gatt_dm *dm, void *context) {
    ...
    err =
        bt_hrs_client_sensor_location_read(&hrs_c, hrs_sensor_location_read_cb);
    if (err) {
        printk("Could not read Heart Rate Sensor location (err %d)\n", err);
    }

    err =
        bt_hrs_client_measurement_subscribe(&hrs_c, hrs_measurement_notify_cb);
    if (err) {
        printk("Could not subscribe to Heart Rate Measurement characteristic "
               "(err %d)\n",
               err);
    }

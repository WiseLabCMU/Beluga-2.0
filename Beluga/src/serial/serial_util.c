 ...
 * @file serial_util.c
 *
 * @brief Taken from zephyr since zephyr won't compile this file in without the
 * shell or uart transport enabled
 *
 * @date 2/20/25
 *
 * @author tom
...
static int mcumgr_serial_decode_frag(struct mcumgr_serial_rx_ctxt *rx_ctxt,
    ...
    rc = base64_decode(rx_ctxt->nb->data + rx_ctxt->nb->len,
                       net_buf_tailroom(rx_ctxt->nb), &dec_len, frag, frag_len);
    if (rc != 0) {
        return -EINVAL;
    }
...
/**
 * @brief Transmits a single mcumgr packet over serial, splits into multiple
 * frames as needed.
 *
 * @param data                  The packet payload to transmit. This does not
...
int mcumgr_serial_tx_pkt(const uint8_t *data, int len, mcumgr_serial_tx_cb cb) {
    ...
    /*
     * This is max input bytes that can be taken to the frame before encoding
     * with Base64; Base64 has three-to-four ratio, which means that for each
     * three input bytes there are going to be four bytes of output used. The
     * frame starts with two byte marker and ends with newline character, which
     * are not encoded (this is the "-3" in calculation).
     */
    int max_input = (((MCUMGR_SERIAL_MAX_FRAME - 3) >> 2) * 3);

    /* Calculate the CRC16 checksum of the whole packet, prior to splitting it
     * into frames */
    ...
    while (src_off < len) {
        ...
        /*
         * Only the first fragment contains the packet length; the packet
         * length, which is two byte long is paired with first byte of input
         * buffer to form triplet for Base64 encoding.
         */
        if (first) {
        ...
        /*
         * Only as much as fits into the frame can be processed, but we also
         * need to fit in the two byte CRC. The frame can not be stretched and
         * current logic does not allow to send CRC separately so if CRC would
         * not fit as a whole, shrink to_process by one byte forcing one byte to
         * the next frame, with the CRC.
         */
        ...
        if (reminder == 0 || reminder == 1) {
            ...
            /* This is the last frame but the checksum cannot be added, remove
             * the last packet flag until the function runs again with the final
             * piece of data and place the checksum there
             */
            last = false;
        } else if (reminder >= 2) {
            last = true;
        ...
        /*
         * Try to process input buffer by chunks of three bytes that will be
         * output as four byte chunks, due to Base64 encoding; the number of
         * chunks that can be processed is calculated from number of three byte,
         * complete, chunks in input buffer, but can not be greater than the
         * number of four byte, complete, chunks that the frame can accept.
         */
        while (to_process >= 3) {
        ...
        if (last) {
            /*
             * Process the reminder bytes of the input buffer, after sending it
             * in three byte chunks, and CRC.
             */
            switch (len - src_off) {
            case 0:
                raw[0] = (crc & 0xff00) >> 8;
